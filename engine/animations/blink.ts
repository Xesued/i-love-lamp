import type { RGBW, ColorGeneratorOptions, ColorGeneratorFunc, LedMap } from "../types"
...
export function blink(opts: BlinkOptions): ColorGeneratorFunc {
    const { onColor, onDurationMs, offColor = [0, 0, 0, 0], offDurationMs, leds, transitionMs = 100 } = opts;
    let duration = 0
 ...
 * @param tickMs  
 */
function getTickAmount(transitionMs: number, msPerTick: number, startColor: RGBW, endColor: RGBW) {
